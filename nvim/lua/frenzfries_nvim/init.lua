...
vim.api.nvim_create_autocmd("TextYankPost", {
    ...
    group = vim.api.nvim_create_augroup("frenzfries.nvim.yank_group", { clear = true }),
    callback = function()
...
require("lazy").setup({
...

    {
        ...
        -- used for completion, autonations, and signature of Neovim apis
        "folke/lazydev.nvim",
        ft = "lua",
        opts = {
            library = {
                -- load luvit types when the "vim.uv" word is founrd
    ...
    { "Bilal2453/luvit-meta", lazy = true },
    {
        ...
        config = function()
            ...
            vim.api.nvim_create_autocmd("LspAttach", {
                group = vim.api.nvim_create_augroup("frenzfries.nvim.lsp_attach", { clear = true }),
                callback = function(event)
                    -- NOTE: Remember that Lua is a real programming language, and as such it is possible
                    -- to define small helper and utility functions
                    local map = function(keys, functions, desc, mode)
                        mode = mode or "n"
                        vim.keymap.set(mode, keys, functions, { buffer = event.buf, desc = "LSP: " .. desc })
                    end
                    ...
                    map("gd", require("telescope.builtin").lsp_definitions, "[G]oto [D]efination")

                    -- find refrence for the word under the cursor
                    ...
                    if client and client.supports_method(vim.lsp.protocol.Methods.textDocument_documentHighlight) then
                        local highlight_augroup =
                            vim.api.nvim_create_augroup("frenzfries.nvim.lsp_highlight", { clear = false })
                ...
                end, --callback function(event) end
            })
            ...
            -- setup language server manually for rust(rust_analyzer), python(pyright), typescript(denols)
            -- local lspconfig = require("lspconfig")

            -- Rust Official Language server
            -- Install Steps:
            -- 1. Install Rust [curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh]
            -- 2. rustup component add rust-analyzer
            -- lspconfig.rust_analyzer.setup {}

            -- Pyright -- python language server [i.e. npm install -g pyright]
            -- lspconfig.pyright.setup {}

            -- Deno's built-in language server for javascript, typescript
